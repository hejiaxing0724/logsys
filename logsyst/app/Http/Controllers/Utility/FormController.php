...
class FormController extends Controller
{
     ...
     * @return array|json
     */
    public function getTransactionFormJson($form_type = 'transaction', $array = false)
    {
        $form = new Form();
        $custom = $form->where('form', $form_type)->select('json')->first();
        $json = '';
        ...
        $json_event = checkArray('0', event(new \App\Events\ClientTransactionForm($json)));
        if ($json_event) {
            $json = json_encode($json_event);
        }
        if ($array) {
    ...
    public function getDependancy($dependency, $linked_topic = '')
    {
        ...
        switch ($dependency) {
            case 'agents':
                return DB::table('users')->where('role', '=', 'agent')->select('id', 'first_name as optionvalue')->get()->toJson();
            case 'priority':
                $auth_user = \Auth::user();
