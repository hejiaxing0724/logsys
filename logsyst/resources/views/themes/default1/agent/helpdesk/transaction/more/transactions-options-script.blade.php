...
var filterClick = 0;
            ...
            $('.yes').click(function () {
                ...
                if (values == "") {
                    ...
                    if (c_status != 'hard-delete'){
                        var url = '{{url("transaction/change-status/")}}/' + values + '/' + c_status;
                        $.ajax({
                            type: "GET",
                            url: url,
                            dataType: "html",
            ...
            function changeStatus(id, name) {
                $('#myModalLabel').html('{{Lang::get("lang.change-transaction-status-to")}}' + name);
                var msg = "{{Lang::get('lang.confirm-to-proceed')}}";
                    var values = getValues();
                    if (values == "") {
            msg = "{{Lang::get('lang.select-transaction')}}";
            ...
            function showAssign(t_id) {
                if (t_id.length >= 1) {
                    $('#assign_transaction').css('display', 'inline');
                } else {
                    $('#assign_transaction').css('display', 'none');
                }
            }

        $(document).ready(function () {
            //checking merging transactions
            $('#Mergetransactions').on('show.bs.modal', function () {

                // alert("hi");
                $.ajax({
                    type: "GET",
                    url: "{{route('check.merge.transactions',0)}}",
                    dataType: "html",
                    data: {data1: t_id},
                    beforeSend: function () {
                        $('.loader1').css('display','block');
                    ...
                    success: function (response) {
                        ...
                        if (response == 0) {
                            ...
                            var message = "{{Lang::get('lang.select-transactions-to merge')}}";
                            ...
                            $.ajax({
                                url: "{{ route('get.merge.transactions',0) }}",
                                dataType: "html",
                                data: {data1: t_id},
                                beforeSend: function(){
                                    $('.loader1').css('display','block');
            ...
            $('#merge-form').on('submit', function () {
                $.ajax({
                    type: "POST",
                    url: "{!! url('merge-transactions/') !!}/" + t_id,
                    dataType: "json",
                    data: $(this).serialize(),
                    beforeSend: function () {
                        $('.loader1').css('display','block');
            ...
            $('#Assigntransactions').on('show.bs.modal', function() {
            ...
            $('#assign-form').on('submit', function() {
                ...
                $.ajax({
                    type: "PATCH",
                    url: "{{url('transaction/assign')}}",
                    dataType: "html",
                    data: $(this).serialize() + '&transaction_id=' + t_id,
                    beforeSend: function() {
                        $('.loader1').css('display','block');
                        $('.loader').css('display','block');
                        $("#assign_body").hide();
                    ...
                    success: function(response) {
                        ...
                        $('.loader').css('display','none');
                        if (response == 1) {
                            var message = "{!!Lang::get('lang.transaction-assigned-successfully')!!} {!!Lang::get('lang.reload-be-patient-message')!!}"
                            $(".success-message, .success-msg, .get-success, #get-success").html(message);
                            $(".alert-success").show();
