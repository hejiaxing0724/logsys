...
@section('PageHeader')
    <h1 id="header">{{Lang::get('lang.transactions')}}</h1>
@stop

@section('content')
    <div class="well" style="display:none;"></div>
    <div class="box box-primary" ng-controller="CreateTransactionAgent">
        <div class="box-header with-border" id='box-header1'>
            <h3 class="box-title">{!! Lang::get('lang.create_transaction') !!}</h3>
...
@push('scripts')
    ...
    <script>
        ...
        app.controller('CreateTransactionAgent', function($scope,$http, $sce,$window,$compile,Upload,$rootScope,$location,vcRecaptchaService){
            ...
            $http.get("{{url('form/transaction')}}").success(function (data) {

                    for(var i in data[0]){
                        $scope.tree3 = data[0];

                        if(data[0][i].api=="agents"&& !data[0][i].linkHelpTopic) {
                            $http.get("{{url('transaction/form/dependancy?dependency=agents')}}").success(function (agentlist) {
                                data[0][i].options=agentlist;
                            });
                        }
                        // Get URLparams
            ...
            $scope.multiselect=function(x){
                ...
                function broker(key,value){
                    if(typeof value=="object" && value.type=="multiselect"){
                        if(value.unique==x){
                            $http.get("{{url('transaction/form/dependancy?dependency=')}}"+dependancy).success(function (data) {
                                if(data!=null){
            ...
            $scope.nestSelectRTL=function(x){
                ...
                if(typeof x=="object"&&x!=undefined){
                    ...
                    $http.get("{{url('transaction/form/dependancy?dependency=department')}}&linkedtopic="+x.id).success( function (data) {
                        for(var i in data){
                    ...
                    for(var i in $scope.tree3) {
                        if($scope.tree3[i].title=='Description' && $scope.tree3[i].agentRequiredFormSubmit==true){
                            if($(".cke_wysiwyg_frame").contents().find("body").html()){
                                ...
                                //console.log($scope.UserInfo['requester']);
            ...
            $scope.requesterEmail=function(e,y,x){
                ...
                if (e.keyCode === 8 || e.keyCode ===46) {
                    ...
                    setTimeout(function(){
                        ...
                        $http.get("{{url('transaction/form/requester?term=')}}"+charCode).success(function (data) {
